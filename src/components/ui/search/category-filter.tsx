    ...
    return (
        <select className="bg-white border-2 border-gray-200 rounded-2xl p-2 shadow-xl animate-in fade-in slide-in-from-top-4 duration-300" onChange={(e) => handleCategoryChange(e.target.value)}>
            ...
            {categories.map(category => (
                <option key={category.slug} value={category.slug} className={clsx("px-4 py-2.5 rounded-xl text-sm font-medium transition-all", {
                    'bg-linear-to-r from-purple-600 to-blue-600 text-white shadow-lg shadow-purple-500/30': searchParams.get('category') === category.slug,
                    'bg-gray-100 text-gray-700 hover:bg-gray-200': searchParams.get('category') !== category.slug,
                })}>
                    {category.name}
                </option>
            ))}
